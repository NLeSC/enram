...
void calcTexture(unsigned char *teximg,unsigned char *vimage, unsigned char *zimage,
    ...
    for (iAzim=0; iAzim<nAzim; iAzim++) {
        for (iRang=0; iRang<nRang; iRang++) {
            ...
            vmoment1 = 0;
            vmoment2 = 0;
            dbz = (zmeta->zoffset) + (zmeta->zscale) * (zimage[iGlobal]);
            ...
            for (iNeighborhood=0; iNeighborhood<nNeighborhood; iNeighborhood++) {
                ...
                if (vimage[iLocal]==missing || zimage[iLocal]==missing) {
                    continue;
                }
                // FIXME why difference between local and global?
                vmoment1 += vmeta->zoffset + vmeta->zscale * (vimage[iGlobal]-vimage[iLocal]);
                vmoment2 += SQUARE(vmeta->zoffset + vmeta->zscale * (vimage[iGlobal]-vimage[iLocal]));
                dbz += zmeta->zoffset + zmeta->zscale * zimage[iLocal];
                count++;
            ...
            if (count < nCountMin) {
                teximg[iGlobal] = missing;
            }
            else {
                if (textype == TEXCV){
                    tex = 10*log10(sqrt(XABS(vmoment2-SQUARE(vmoment1))))-dbz;
                    if (tex < texmeta->zoffset+texmeta->zscale) {
                        tex = texmeta->zoffset+texmeta->zscale;  // FIXME not sure this is right
                    }
                }
                if (textype == TEXSTDEV){
                    tex = sqrt(XABS(vmoment2-SQUARE(vmoment1)));
                }
                teximg[iGlobal] = ROUND((tex-texmeta->zoffset)/(texmeta->zscale));
            } // else
        } // for iRang
    } // for iAzim
}// calcTexture
