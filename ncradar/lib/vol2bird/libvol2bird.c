...
/******************************************************************************/
/*This function computes a texture parameter based on a block of (nRangLocal x*/
/* nAzimLocal) pixels. The texture parameter equals the local standard deviation*/
/*in the velocity field.                                                      */
/******************************************************************************/
// Implementation of native method calcTexture() of RadarScan class
JNIEXPORT void JNICALL Java_nl_esciencecenter_ncradar_RadarScan_calcTexture(JNIEnv *env, jobject thisObj, int tMissing, int tnAzim, int tnRange, double tOffset, double tScale, int vImage[], int vMissing, int vnAzim, int vnRange, double vOffset, double vScale, int zImage[], int zMissing, int znAzim, int znRange, double zOffset, double zScale, int nRangLocal, int nAzimLocal, int nCountMin, int textype) {
    ...
    for (iAzim=0; iAzim<nAzim; iAzim++) {
        for (iRang=0; iRang<nRang; iRang++) {
            ...
            for (iNeighborhood=0; iNeighborhood<nNeighborhood; iNeighborhood++) {
                ...
                if (vImage[iLocal]==vMissing || zImage[iLocal]==zMissing) {
                    continue;
                }
                // FIXME why difference between local and global?
                vRad = vOffset + vScale * (vImage[iGlobal]-vImage[iLocal]);
                vmoment1 += vRad;
                vmoment2 += SQUARE(vRad);
                // FIXME dbz at iRang,iAzim is counted double?
                dbz += zOffset + zScale * zImage[iLocal];
                count++;
            ...
            if (count < nCountMin) {
                ...
                tImage[iGlobal] = ROUND((tex-tOffset)/(tScale));
            } // else
        } // for iRang
    } // for iAzim
    return;
