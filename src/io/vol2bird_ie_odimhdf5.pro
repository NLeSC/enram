;
;IDL routine to read ODIM hdf5 radar files from Ireland
;
;Author: M. de Graaf (KNMI)
;Date: April 2012
;

PRO vol2bird_ie_odimhdf5
  ;
  ;Call  
  ;
  ; RESOLVE_ROUTINE, 'volbird_ie_odimhdf5', /compile_full_file
  ;
  ...
  ; (e.g. vol2bird_ie_odimhdf5_read_scan for the scan read procedure)
...
END
;
PRO vol2bird_ie_odimhdf5_get_datetime,file,this_date,this_time
;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
PRO vol2bird_ie_odimhdf5_read_scan, 	$
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
;IN Irish format, the different data products are scattered over several files, 
;one data product per file.
;
;This routine can be called with the name of a file containing one data product
;while requesting another data product. If the file containing the requested 
;product is available in the same folder, it will be retrieved from that file.
;
;First determine which filename is passed.
passed_file=h5infile
passed_type='A'
IF STRPOS(passed_file,'PAIZ') ne -1 THEN BEGIN
  strstart=STRPOS(passed_file,'PAIZ') 
  passed_type='W'
ENDIF  
IF STRPOS(passed_file,'PAHZ') ne -1 THEN BEGIN
  strstart=STRPOS(passed_file,'PAHZ')
  passed_type='V'
ENDIF
IF STRPOS(passed_file,'PAGZ') ne -1 THEN BEGIN 
  strstart=STRPOS(passed_file,'PAGZ')
  passed_type='Z'
ENDIF
...
ENDIF
;
requested_type=STRUPCASE(type)
CASE requested_type OF
'W' : requested_file='PAIZ'
'V' : requested_file='PAHZ'
'Z' : requested_file='PAGZ'
ELSE : requested_file='XXXX'
ENDCASE
;
;INSERT the correct code into the filename to open the requested type
IF requested_type ne passed_type and requested_file ne 'XXXX' THEN $
   STRPUT,h5infile,requested_file,strstart
;
;REMEMBER which date/time has been processed, so it can be passed back, 
;to avoid multiple passing of file with the same date/time 
dt_strstart=STRPOS(FILE_BASENAME(passed_file),'C_EIDB_')+7
thisdatetime=STRMID(FILE_BASENAME(passed_file),dt_strstart,14)
this_code=STRMID(passed_file,50,3)
;
;CHECK if all products will be available, otherwise quarantine the 
;few files that are available.
all_files_string=passed_file
STRPUT,all_files_string,'PA*Z',strstart
allfiles=FILE_SEARCH(all_files_string,count=allcount)
necessary_products=0
FOR i=0,allcount-1 DO BEGIN
  IF STRPOS(allfiles[i],'PAHZ') ne -1 THEN necessary_products += 1
...
h5infile=passed_file	    	    ;;MAKE sure the same filename is returned.
;
  ...
  WHILE not read DO BEGIN
   variable+=1
	...
	ENDIF
      END
      ELSE: BEGIN
         PRINT, "Please select a type 'Z','U','V',or 'W' in read_odimh5_scan'
	 STOP,"Check code"
      END
    ENDCASE
    ;
    ...
    IF KEYWORD_SET(imgflag) THEN BEGIN
      ;
      ;ALSO load the other files, containing other quatities.
      tmp_h5infile = h5infile
      all_requests = ['PAGZ','PAHZ','PAIZ']
      ;      
      FOR ireq = 0,N_ELEMENTS(all_requests)-1 DO BEGIN 
           ;
	   ;OPEN the file and load its product
	   STRPUT,tmp_h5infile,all_requests[ireq],strstart 
	   tmp_h5infile=FILE_SEARCH(tmp_h5infile,count=count)
	   IF count eq 0 THEN CONTINUE
	   tmp_id=H5F_OPEN(tmp_h5infile)    	 
	   ;print, tmp_h5infile
	   object=STRING(FORMAT='("/dataset",i0,"/data",i0)',iscan,variable)
	   tmp_data=H5_PARSE(tmp_id,object)
	   this_quantity=tmp_data.what.quantity._data
	   
	   find_quantity=WHERE(STRUPCASE(this_quantity) eq ['TH','DBZH','VRAD'],bird_quantity)
	   IF bird_quantity ne 0 THEN BEGIN
	     ;
	     object=STRING(FORMAT='("/dataset",i0,"/data",i0)',iscan,variable)
	     scan_data=H5_PARSE(tmp_id,object,/READ_DATA)
	     ;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
PRO vol2bird_ie_odimhdf5_read_cluttermap,h5cluttermap,cmmeta,cmscan
;
COMMON constants
;
make_meta,cmmeta
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
PRO vol2bird_ie_odimhdf5_write_profile,     $
h5outfile,in,	    	    	    	    $
success=success,    	    	    	    $
vmeta,zmeta,uzmeta,   	    	    	    $
