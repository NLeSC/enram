;
;IDL routine to read ODIM hdf5 radar files from Finland
;
;Author: M. de Graaf (KNMI)
;Date: April 2012
;

PRO vol2bird_fi_odimhdf5
  ;
  ;Call  
  ;
  ; RESOLVE_ROUTINE, 'vol2bird_fi_odimhdf5', /compile_full_file
  ;
  ...
  ; (e.g. vol2bird_fi_odimhdf5_read_scan for the scan read procedure)
...
END
;
PRO vol2bird_fi_odimhdf5_get_datetime,file,this_date,this_time
;
IF SIZE(file[0],/TYPE) ne 7 THEN BEGIN
  PRINT, 'please, provide a string (array) of filename(s)'
  stop
ENDIF
this_date=STRMID(FILE_BASENAME(file),11,8)
this_time=STRMID(FILE_BASENAME(file),19,4)+'00'
;
END
;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
PRO vol2bird_fi_odimhdf5_read_scan, 	$
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
;IN Finnish format, the different scans are scattered over several files, 
;one scan per file.
;
;This routine can be called with the name of a file containing one scan.
;all other scans available in the same folder will be retrieved as well.
;
;First determine which filename is passed.
;
passed_file=h5infile
this_path=FILE_DIRNAME(passed_file,/MARK_DIRECTORY)

;GET the date / time string, this will be used to find all
;other files/scans.

filebase=FILE_BASENAME(passed_file,'.h5')
this_code=STRMID(filebase,0,3)
thisdatetime=STRMID(filebase,11,12)
;
allfiles = FILE_SEARCH(this_path+this_code+'*'+thisdatetime+'*',count=nscan)
;
;ASSIGN a scanid to all files, according to their character numbering, 
;since not all scans begin at A (1) and not all scans are necessarily available
;
fi_scan_id = '_'+['A','B','C','D','E','F','G','H','I','J','K','L','M','N']+'_'
allfiles=allfiles[SORT(allfiles)]
fi_sort = INTARR(N_ELEMENTS(fi_scan_id))-1
FOR ifile=0,N_ELEMENTS(allfiles)-1 DO BEGIN
  FOR j=0,N_ELEMENTS(fi_scan_id)-1 DO BEGIN
    IF STRPOS(allfiles[ifile],fi_scan_id[j]) ne -1 THEN BEGIN
      fi_sort[j]=ifile+1
    ENDIF
  ENDFOR
ENDFOR
;
IF nscan eq 0 THEN BEGIN
  PRINT, 'No files (= scans in Finnish format) found for date/time '+thisdatetime  
  RETURN
ENDIF
;
...
IF N_PARAMS() eq 5 THEN BEGIN
  ...
  ;
  in=CREATE_STRUCT("what",what,"where",where)
  ;
  H5G_CLOSE,gid
...
IF KEYWORD_SET(scanid) THEN BEGIN
  ;
  IF scanid le 0 OR scanid gt Nscan THEN BEGIN 	    ;;CHECK given scanid
     MESSAGE,'Selected scan group ('+STRTRIM(scanid)+') is out of range!'
     RETURN
  ENDIF   
  ;  
  IF WHERE(scanid eq fi_sort) eq -1 THEN BEGIN
     PRINT,'Selected scan group is not available'
     RETURN
  ENDIF   
  ;
  beginscan=fi_sort[scanid-1]
  endscan=fi_sort[scanid-1]
  Nscan=1
ENDIF ELSE BEGIN
  beginscan=1
  endscan=WHERE(fi_sort eq MAX(fi_sort))+1
  endscan=endscan[0]
  Nscan=endscan
ENDELSE
;
...
FOR iscan=beginscan,endscan DO BEGIN
  ;
  IF fi_sort[iscan-1] eq -1 THEN BEGIN
    scandata=CREATE_STRUCT(scandata,"scan"+STRTRIM(iscan,2),BYTE(0))	;;UPDATE the IDL structure
    success=1L
    CONTINUE
  ENDIF
  ;
  h5_id=H5F_OPEN(allfiles[fi_sort[iscan-1]-1])    ;;OPEN the current HDF5 scan file
  ;print, 'opening '+allfiles[fi_sort[iscan-1]-1]
  ; 
  ...
  ;
  what=H5_PARSE(scan_id,"/dataset1/what")
  meta[iscan].date=what.startdate._DATA
  meta[iscan].time=what.starttime._DATA
  where=H5_PARSE(scan_id,"/dataset1/where")
  ...
  variable=0
  ;
  scan_id = H5G_OPEN(h5_id,'/dataset1')
  numdata=0     ;FIND number of data groups in this scan/dataset
  for i=0, H5G_GET_NUM_OBJS(scan_id)-1 DO $
    IF 'data' eq STRMID(STRLOWCASE(H5G_GET_MEMBER_NAME(scan_id,'/dataset1',i)),0,4) THEN numdata+=1
  ;  
  WHILE not read DO BEGIN
   variable+=1
    ;
    object=STRING(FORMAT='("/dataset1/data",i0)',variable)
    dset=H5_PARSE(scan_id,object)
    ;
    ;HANDLE a common error of missing data
    ;
    ;IF something is missing
    thetags=tag_names(dset)
    itag=WHERE(STRUPCASE(thetags) eq 'WHAT',nwhat)
    ;
    ;remove all scans
    IF nwhat eq 0 THEN BEGIN
      SPAWN,"date +%A', '%e' '%B' '%Y,' '%R:%S': '", todate  
      message=STRING(FORMAT='(a,"Bad scan encountered in ",a,", quarantining all       ")',$
        	 todate,FILE_BASENAME(allfiles[fi_sort[iscan-1]-1]))
      PRINT, message
		 
      FOR ifile = 0,N_ELEMENTS(allfiles)-1 DO BEGIN
      
        quarantine_dir='/usr/people/graafdem/FLYSAFE/process/data/odim/FI_'+STRLOWCASE(this_code)+'/quarantined/'
        mvstring = STRING(FORMAT='("mv ",a," ",a)',allfiles[ifile],quarantine_dir)
        SPAWN, mvstring
      ENDFOR		 
      ;
      scan=0
      ...
      success=0
      ;      
      processed_datetime=thisdatetime ;SKIP this date from now on
      ;
      RETURN
      ;
    ENDIF
    ...
    CASE  STRUPCASE(type) OF 
      'Z' : BEGIN
  	  ...
  	  meta[iscan].zscale=dset.what.gain._data*256
	...
	ENDIF
      END
      'R' : BEGIN
        IF STRUPCASE(quantity) eq 'RHOHV' THEN BEGIN
  	  meta[iscan].zscale=dset.what.gain._data*256
	  meta[iscan].zoffset=dset.what.offset._data
	  meta[iscan].missing=LONG(dset.what.undetect._data)
	  read=1
    ...
    IF read THEN BEGIN
      ...
      scandata=CREATE_STRUCT(scandata,"scan"+STRTRIM(iscan,2),BYTE(scan_data.DATA._DATA))	;;UPDATE the IDL structure
      success=1L
      ;
    ENDIF  
    ;
    ...
    IF KEYWORD_SET(imgflag) THEN BEGIN
      ...
      IF bird_quantity ne 0 THEN BEGIN
	;
	...
	IF not KEYWORD_SET(data_n) THEN BEGIN
	  ...
	  ;
	  ;CHANGE the internal name of the structure	       
	  data_n.(0)._NAME="/dataset"+STRTRIM(iscan,2)+"/data1"
	;OR update it  
	ENDIF ELSE BEGIN
	  ...
	  ;
	  ;CHANGE the internal name of the structure	       
	  data_n.(idata-1)._NAME="/dataset"+STRTRIM(iscan,2)+"/data"+STRTRIM(idata,2)
	ENDELSE  
	;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
PRO vol2bird_fi_odimhdf5_read_cluttermap,h5cluttermap,cmmeta,cmscan
;
COMMON constants
;
make_meta,cmmeta
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;
PRO vol2bird_fi_odimhdf5_write_profile,     $
h5outfile,in,	    	    	    	    $
success=success,    	    	    	    $
vmeta,zmeta,uzmeta,   	    	    	    $
