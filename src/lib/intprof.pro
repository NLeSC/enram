FUNCTION INTPROF, data, RAW=raw, _EXTRA=ex
...
DBZFACTOR=360.0    ;factor for conversion dBZ to reflectivity
...
ENDIF
if n_elements(raw) le 0 then raw = 0
...
CASE tag_names(data,/STRUCTURE_NAME) OF
  'DATFLEDER':BEGIN
     ...
     if n_elements(index) eq 1 then return,[[0],[0]]
  ...
  'DATVP2':BEGIN
     ...
     if raw then index=WHERE(data.height GT HEIGHT_RANGE[0] and data.height LT HEIGHT_RANGE[1])
     if n_elements(index) eq 1 then return,[[0],[0]]
     values=LAYER_HEIGHT*((10^(data.dbz/10+ALOG10(DBZFACTOR)))[index])/SIGMA_BIRD
     if raw then values=LAYER_HEIGHT*((10^(data.dbzraw/10+ALOG10(DBZFACTOR)))[index])/SIGMA_BIRD
     jultimes = (data.juldate)[index]
   END
...
notimes=n_elements(uniqtimes)
novalues=n_elements(values)

;************************************************************************************
; compute vertically integrated bird densities
;************************************************************************************
if (notimes eq 1) then RETURN,[[uniqtimes],[total(values)]]
