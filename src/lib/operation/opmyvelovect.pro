...
;-
;
PRO OPMYVELOVECT,U,V,X,Y, Missing = Missing, Length = length, Dots = dots,  $
        CLIP=clip, NOCLIP=noclip, OVERPLOT=overplot, NOSCALE=noscale,$
	AUTOCOLOR=autocolor, VMAX=vmax, WINDBARB=windbarb, ASPECT=aspect, _REF_EXTRA=extra
...
;
        ...
        if n_elements(vmax) le 0 then vmax = 40
        if n_elements(length) le 0 then length = 1.0
        if n_elements(noscale) le 0 then noscale = 0
        if n_elements(autocolor) le 0 then autocolor = 0
        ...
        endelse
        if not (n_elements(good) gt 1) then begin
           if (good lt 0) then return
        endif
        ugood = u[good]
        vgood = v[good]
	maggood=1.94384449*mag[good] ;select and convert to knots
	if (n_elements(where(u eq 0)) gt 1) then u[where(u eq 0)]=1E-15 ;prevent division by zero
	direc=360*atan(ugood,vgood)/(2*!pi)+180
    ...
    vectorcolors=bytscl(maggood,MIN=0,MAX=vmax,TOP=!d.table_size-1)
    if (not autocolor) then vectorcolors=0*vectorcolors+!P.COLOR
    
    maxmag=max([max(abs(ugood/x_step)),max(abs(vgood/y_step))])
    if noscale then maxmag=1
...
;
        for i=0L,n_elements(good)-1 do begin     ;Each point
                ...
                yd=y_step
		col=vectorcolors[i]
		if (windbarb) then begin
                   opmywindbarb,x0,y0,maggood[i],direc[i],color=col,length=length,clip=clip,aspect=aspect
	        endif else begin
                   plots,[x0,x1,x1-(ct*dx/xd-st*dy/yd)*xd, $
                      x1,x1-(ct*dx/xd+st*dy/yd)*xd], $
                      [y0,y1,y1-(ct*dy/yd+st*dx/xd)*yd, $
                      y1,y1-(ct*dy/yd-st*dx/xd)*yd], $
                      clip=clip,noclip=noclip,_EXTRA=extra,color=col
		endelse
        endfor
