...
PRO OPMyWindbarb, x, y, wspeed, wdirection, $
   Aspect=aspect, $
   Clip=clip, $
   Color=color, $
   Length=length, $
   ...
   FOR j=0L, N_Elements(x) - 1 DO BEGIN
      ...
      PLOTS, [x1, x2], [y1,y2], /Normal, Color=color[j], Clip=clip, Thick=thick
      ...
      FOR i = 1, num10 DO BEGIN
         ...
         PLOTS, [x1, x2, x3], [y1, y2, y3], /Normal, Color=color[j], Clip=clip, Thick=thick
         x1 = x2
         y1 = y2
      ENDFOR
      ...
      FOR i = 1, num50 DO BEGIN
         ...
         IF y2 LT clip[1] OR y2 GT clip[3] THEN CONTINUE
         PLOTS, [x2, x0], [y2, y0], /Normal, Color=color[j], Clip=clip, Thick=thick
         POLYFILL, [x1, x2, x3], [y1, y2, y3], /Normal, Color=color[j], Clip=clip
