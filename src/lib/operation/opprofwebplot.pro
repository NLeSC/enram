...
PRO OPPROFWEBPLOT, datbilt, startdate, enddate, _EXTRA=ex
...
;****************************************************************************
  ...
  print, '*************************************************************************************'
   ...
   IF (N_ELEMENTS(ex) NE 0) THEN BEGIN
     ...
     IF WHERE(TAG_NAMES(ex) eq 'FILE') NE -1 THEN FILE=ex.FILE ELSE FILE='opwebplot.ps'
   ENDIF ELSE BEGIN
     TRESH_STDEV=2
     TRESH_PRECIP=1
     SIGMABIRD=10
   ...
   ;Device, _Extra=PSConfig(/NoGUI, /portrait, /european, Filename='webplot.ps')
   ...
   opplotprof, datbilt.juldate,datbilt.height,stdevplot,datbilt.u,datbilt.v,/windbarb,NVectors=[30,10],vectorlength=0.02,$
      ...
      XPOS=0,YTICKLEN=1,XTICKLEN=1,CONTOURRANGE=[0.8,3.8],XSTYLE=1,POSITION=[0.1,0.55,0.8,0.9],XTICKUNITS='Time',BIRDTICKS=1
   ...
   plot,datbiltint2[*,0],datbiltint2[*,1], XTICKFORMAT = 'LABEL_DATE',$
     ...
     XSTYLE=1,YSTYLE=1,XTICKLEN=1,POSITION=[0.1,0.05,0.8,0.40]
