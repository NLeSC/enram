...
%
%  Limitations:
%    Does not yet work for extended file pointer.
...
%----------------------------------------------------------
	...
	if (nargin < 2)
	  ...
	  disp(' val = resget(resfile,tag)')
	  disp('   resfile = name of resfile.')
	  disp('   tag = tag name of item to get.')
	  disp('   val = returned value.')
	  disp(' ')
