...
;
ff=REPLICATE(0.,NDATA)
dd=REPLICATE(0.,NDATA)
;
texmeta=vmeta
...
FOR l=0,Nlev-1 DO BEGIN
   FOR k=0,NDATA-1 DO BEGIN
      ff[k]=sqrt(udata[k,l]*udata[k,l]+vdata[k,l]*vdata[k,l]);
      dd[k]=atan2(-udata[k,l],-vdata[k,l])*!RADEG;
      IF (dd[k] lt 0) THEN  dd[k]+=360;
   ENDFOR
...
;
FOR l=0,Nlev-1 DO BEGIN
  zdata[0,l]= (xdata[0,l] gt stdevbird) ? DBZFACTOR*10^(zdata[0,l]/10) : 0;
ENDFOR
;
